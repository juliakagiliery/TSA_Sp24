...
```{r}
library(readxl)
DataPandC <- read_excel("~/Julia_Kagiliery_TSA_Sp24/Data/Table_10.1_Renewable_Energy_Production_and_Consumption_by_Source.xlsx", skip = c(10), sheet="Monthly Data",col_names=TRUE) 
```

```{r}
DataPandC <- DataPandC[,4:6]
...
```{r}
DataPandCTS <- DataPandC |>
    ts(start = c(1973,1), frequency = 12, deltat = 1/12) 
```
...
for(i in 1:609){
  ...
  Acf(DataPandCTS[,i],lag.max=40,main=paste("Total Biomass Energy Production",i,sep="")) 
  # because I am not storing Acf() into any object, I don't need to specify plot=TRUE 
  Pacf(DataPandCTS[,i],lag.max=40,main=paste("Total Biomass Energy Production",i,sep=""))
}
```
