...
```{r}
DataPandC <- read_excel("~/Julia_Kagiliery_TSA_Sp24/Data/Table_10.1_Renewable_Energy_Production_and_Consumption_by_Source.xlsx", skip = 10, sheet="Monthly Data",col_names=TRUE) 
```
...
```{r}
  P1ACF <- Acf(DataPandCTS[,2],lag.max=40,main=paste("Total Biomass Energy \n Production"), plot = FALSE)
  
  P2PCF <- Pacf(DataPandCTS[,2],lag.max=40,main=paste("Total Biomass Energy \n Production"), plot = FALSE)
...
```{r}
  P4ACF <- Acf(DataPandCTS[,2],lag.max=40,main=paste("Total Renewable Energy \n Production"), plot = FALSE)
  
  P5PCF <- Pacf(DataPandCTS[,2],lag.max=40,main=paste("Total Renewable Energy \n Production"), plot = FALSE)
...
```{r}
  P7ACF <- Acf(DataPandCTS[,3],lag.max=40,main=paste("Total Hydroelectric Energy \n Consumption"), plot = FALSE)
  
  P8PCF <- Pacf(DataPandCTS[,3],lag.max=40,main=paste("Total Hydroelectric Energy \n Consumption"), plot = FALSE)
