...
library(lubridate)
library(tidyverse)
library(cowplot)#load this package so yon clean the data frame using pipes
```
...
```{r}
print(decompose(WindTS, type = "additive"))
```

```{r}
print(decompose(SolarTS, type = "additive"))
```
...
```{r}
print(decompose(WindTS, type = "multiplicative"))
```

```{r}
print(decompose(SolarTS, type = "multiplicative"))
```
...
```{r}
original_start_date <- start(TSFilterWind)
print(original_start_date)
```

```{r}
print(decompose(TSFilterWind, type = "additive"))
```

```{r}
print(decompose(TSFilterSolar, type = "additive"))
```
...
```{r}
p1 <- autoplot(CleanWindTS)
```

```{r}
p2 <- autoplot(CleanSolarTS)
```
...
```{r}
p5 <- autoplot(TSCSolar)
p6 <- autoplot(TSCWind)
p7 <- autoplot(TSFilterWind)
p8 <- autoplot(TSFilterSolar)
```

```{r}
plot_grid(p5, p8, p6, p7, nrow = 2)
```
