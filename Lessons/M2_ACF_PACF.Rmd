...
```{r}
#using package lubridate (makes it into an object)
my_date <- paste(raw_inflow_data[,1],raw_inflow_data[,2],sep="-") # [,1] ccopies the first column only
my_date <- my(my_date)  #function my from package lubridate, my means month year
head(my_date)
...
```{r acf}
# Option 1: The function acf() and pacf() is from package "stats", acf() is the auto corelation function
